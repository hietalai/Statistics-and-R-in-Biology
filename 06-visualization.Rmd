...
- <span class="terminology">Diagramyta</span>: Det område som innehåller all information tillhörande en visualisering
- <span class="terminology">Rityta</span>: Det område som innehåller det faktiska data som visualiseras
- <span class="terminology">Axel</span>: Kanterna som begränsar ritytan, ofta benämnt som x- och y-axel för den vågräta ( - ) respektive lodräta ( | ) axeln
- <span class="terminology">Axelförklaring</span>: En rubrik som beskriver vad den angivna axeln visar för information
- <span class="terminology">Skalvärden</span>: Steg som anger specifika värden på den angivna axeln
- <span class="terminology">Stödlinjer</span>: Linjer vilka agerar som en förlängning av axlarnas skalvärden i bakgrunden av ritytan
- <span class="terminology">Titel/rubrik</span>: En rubrik för diagrammet
- <span class="terminology">Källhänvisning</span>: En text placerade i någon av de nedre hörnen som anger en källa för det visualiserade datamaterialet om sådan finns

## Ett diagram av hög kvalité
...
Ett <span class="terminology">stapeldiagram</span> lämpar sig för att visualisera fördelningen av kvalitativa variabler. Eftersom en kvantitativ variabel omfattar många olika värden kommer frekvenserna aldrig bli nog stora, och antalet värden som variabeln kan anta vara alltför många.

```{r echo = FALSE, fig.cap = "Exempel på stapeldiagram för en kvalitativ (t.vä.) och kvantitativ (t.hö.) variabel", fig.width = 8}

p1 <- ggplot(penguins) + aes(x = species) + geom_bar(width = 0.6) +
  labs(y = "Antal", x = "Art") 

p2 <- ggplot(penguins) + aes(x = bill_length_mm) + geom_bar() +
  labs(y = "Antal", x = "Näbblängd") 
...
```{r echo = FALSE, fig.cap = "Exempel på diagram med bra rityta (t.vä.) och för liten rityta (t.hö.)", fig.width = 8}

p1 <- ggplot(penguins) + aes(x = species) + geom_bar(width = 0.6) +
  labs(y = "Antal", x = "Art") + theme(panel.border = element_rect(color = "red"))

p2 <- ggplot(penguins) + aes(x = species) + geom_bar(width = 0.6) +
  labs(y = "Antalet pingviner av olika arter", x = "Art") + theme(panel.border = element_rect(color = "red"))
...
```{r echo = FALSE, fig.cap = "Exempel på stödlinjer som ligger i bakgrunden (t.vä.) och stödlinjer som tar onödig fokus från informationen (t.hö.)", fig.width = 8}
...
ggplot(penguins) + aes(x = species) + geom_bar(width = 0.6) +
  labs(y = "Antal", x = "Art", caption = "Källa: Gorman, Williams, Fraser (2014)")
...
```{r exampleBar, echo = FALSE, fig.cap = "Ett stapeldiagram som visar en fördelning av en kvalitativ variabel", fig.width = 4}
...
```{r echo = FALSE, fig.cap = "Ett stapeldiagram som visar ett aggregerat kvantitativt värde grupperat på en kvalitativ variabel", fig.width = 4}

penguins %>% 
  group_by(species) %>% 
  summarize(
    Medelvikt = mean(body_mass_g)
  ) %>% 
  ggplot() + aes(x = species, y = Medelvikt) + geom_bar(stat = "identity", width = 0.6) + 
  labs(x = "Art")
...
Paketets visualiseringar utgår ifrån en `tibble` som vi antingen skapat själv eller laddat in från en fil från hårddisken. Vi kommer utgå ifrån `penguins` som laddades in i [detta kapitel](#example). Glöm inte att rensa bort de saknade värdena i materialet med hjälp utav `filter()`.

#### Grundkomponenter 
Vi kan nu börja med att skapa figur \@ref(fig:exampleBar). Vi börjar med de tre grundkomponenterna av ett `ggplot`-diagram; `ggplot()`, `aes()` och `geom_x()`. Alla diagram måste innehålla dessa tre komponenter i någon form för att vi ska kunna producera något överhuvudtaget, sen kan vi lägga till andra instruktioner för att ändra diagrammet utseende.
...
htmltools::div(
  ...
  htmltools::HTML("För att y-axeln ska visa relativa frekvenser istället för absoluta, kan vi i <pre>geom_bar()</pre> lägga till argumentet <pre>mapping = aes(y = (count/sum(count)) %>% after_stat())</pre>. Diagrammet ändrar sig inte i sin form, staplarna är fortfarande lika höga i relation till varandra, men tolkningar av detta diagram kan nu göras i andelar (procent) istället för antal.")
  )
)
```

Med dessa grundkomponenter får vi fram ett diagram, men vi kan väl alla hålla med om att det i detta läge inte ser särskilt snyggt och tydligt ut. Ett snabbt och enkelt sätt att få till lite snyggare diagram är att använda någon utav `ggplot`s teman som finns tillgängliga genom `theme_X()`. Exempelvis är ett stilrent tema att utgå ifrån `theme_bw()` likt:
...
2. Alternativet är att visa den relativa fördelningen grupperat på grupperingsvariabeln, alltså att vardera kategoris staplar summerar var för sig till 100 procent. Vi behöver då revidera koden som anger beräkningen till relativa frekvenser till: `aes(y = after_stat(count/tapply(count, x, sum)[x]))`.
...
p2 <- ggplot(penguins) + aes(x = island, fill = species) + 
  geom_bar(aes(y = after_stat(count/tapply(count, x, sum)[x])), position = "dodge") + 
  theme_bw()
...
penguins_extrem <- penguins %>% 
  filter(abs(bill_depth_mm - predict(lm(bill_depth_mm ~ bill_length_mm, data = penguins))) < 12000)

penguins_extrem_points <- penguins %>% 
  filter(abs(bill_depth_mm - predict(lm(bill_depth_mm ~ bill_length_mm, data = penguins))) > 12000)
...
<!-- tidsserie_exempel <- read.csv2("datasets/tidsserie.csv", encoding = "UTF-8") -->
...
<!-- Datamaterialet som används till detta exempel är ett utav de inbyggda materialen som finns i R, nämligen `penguins`. Det innehåller fyra stycken kontinuerliga variabler som beskriver olika mått på blommor och ytterligare en kategorisk variabel som anger vilken art observationen tillhör. Datamaterialet ser ut som följer: -->

<!-- ```{r penguins, echo = FALSE, eval = TRUE} -->

<!-- kable(head(penguins, n = 5), format = "html", caption = "Första observationerna i penguins-data") %>% -->
<!--   kable_styling(position = "center", full_width = FALSE) -->
